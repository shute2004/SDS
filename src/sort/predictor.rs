...
impl<'a> Predictor<'a> {
    ...
    pub fn new(samples: &'a [f64], inv_widths: &'a [f64], bucket_count: usize) -> Self {
        ...
        let bucket_scale = if sample_last > 0 { bucket_last as f64 / sample_last as f64 } else { 0.0 };
        Self { samples, inv_widths, sample_last, bucket_last, bucket_scale, min_sample, max_sample, inv_span }
    }

    #[inline(always)]
    pub fn predict(&self, x: f64) -> usize {
        if self.bucket_last == 0 || self.sample_last == 0 { return 0; }
        if x <= self.min_sample { return 0; }
        if x >= self.max_sample { return self.bucket_last; }
    ...
    fn upper_bound(&self, x: f64) -> usize {
        ...
        let (mut left, mut right) = if probe_value <= x { (probe + 1, self.samples.len()) } else { (0, probe) };
        while left < right {
            let mid = (left + right) >> 1;
            let value = unsafe { *self.samples.get_unchecked(mid) };
            if value <= x { left = mid + 1; } else { right = mid; }
        }
        left
    }
}
